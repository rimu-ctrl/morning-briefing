"""
OpenAI API(gpt-4o-mini)로 기사의 한국어 핵심 요약(5줄 이내)을 생성합니다.
URL 기반 파일 캐시를 사용해 동일 기사는 재생성하지 않습니다.
OPENAI_API_KEY가 없으면 조용히 빈 문자열을 반환합니다.
"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    "당신은 IT업계 UX/UI 디자이너에게 매일 아침 업계 동향을 brief하는 시니어 에디터입니다.\n"
                    "다음 기사의 핵심을 한국어로 요약해주세요.\n"
                    "- 5줄 이내의 bullet point, 각 줄은 '• '로 시작\n"
                    "- 직역 투(번역체) 표현 없이, 사람이 말하듯 자연스럽고 매끄러운 문장으로 작성\n"
                    "- 단순 사실 나열에 그치지 말고, 그게 왜 중요한지·무엇이 바뀌는지의 인사이트를 짧게 짚어줄 것\n"
                    "- 군더더기 없이 핵심만 간결하게\n\n"
